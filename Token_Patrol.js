...
function showmenu(){	
	...
	desc += "[Set Position](!tp --setPosition) \n";
	desc += "[Set End Position](!tp --setEndPosition) \n \n \n";
	
	desc += "[Start Patrolling](!tp --startPatrol) \n";
	desc += "[Stop Patrolling](!tp --stopPatrol) \n \n";
	
	desc += "[Set Patrol Speed](!tp --setPatrolSpeed ?{Seconds between each movement?|1}) \n \n \n"
	
	desc += "[Toggle Patrol Path](!tp --togglePatrolPath) \n \n \n";
	
	desc += "[Clear Current Patrol Points](!tp --clearPatrol) \n \n \n";
	
	desc += "[Save Current Patrol Points](!tp --savePatrol) \n";
	desc += "[Load Patrol Points](!tp --loadPatrol) \n \n";
	
	desc += "[Toggle Logging](!tp --toggleLogging) \n \n";
	
	sendChat("Token Patrol", "/w gm &{template:desc} {{desc=" + desc + "}}");
}
...
function createPatrolPaths(){
	...
	for(let i = 0; i < patrollingTokens.length; i++){
		...
		if(tokenLeft === undefined && tokenTop === undefined){
			if(chatLogging){
				sendChat("Token Patrol", "/w gm &{template:desc} {{desc=No Patrol to Show}}");
				return;
			}
		}
...
function clearVars(){	
	...
	if(chatLogging){
		sendChat("Token Patrol", "/w gm &{template:desc} {{desc=All current Patrol Points have been cleared}}");
	}
}
...
function savePatrol(){	
	...
	if(chatLogging){
		sendChat("Token Patrol", "/w gm &{template:desc} {{desc=All current Patrol Points have been saved}}");
	}
}
...
function loadPatrol(){
	...
	if(chatLogging){
		sendChat("Token Patrol", "/w gm &{template:desc} {{desc=Patrol Points have been loaded}}");
	}
}
...
function addPatrollingToken(){
	...
	if(hasEndPoint){
		...
		if(chatLogging){
			sendChat("Token Patrol", "/w gm &{template:desc} {{desc=New End Point Added for "+ selectedToken.get("name") + "}}");
		}
		hasEndPoint = false;
	}else{		
		if(chatLogging){
			sendChat("Token Patrol", "/w gm &{template:desc} {{desc=New Patrol Point Added for "+ selectedToken.get("name") + "}}");
		}
	}	
